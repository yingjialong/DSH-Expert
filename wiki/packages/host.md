...
- direct`IApiClient.sessions.create`成功后，Host在`session/created`发`host/session-added`并携带当时effective`agentPreset`；create response也echo它。stream frame与unary response无先后保证，`session.list`是重连/显式refresh baseline而非push ack。
- list presence是current visibility正证；一次absence不是linearizable not-committed证明：create可能仍在setup/publication，或旧generation成功的fresh blank Session在first-party lazy persistence未materialize便遇Host restart。rc.2无create occurrence/commit-status/getHeader query；禁止幂等重放时absence必须保留unknown。`session.history`可区分current`session-not-found`与`internal`storage failure，但不接受expected preset，也不证明历史create outcome。
...
- **`session.create`的live publication、Session artifact与Workspace记账是三个边界**：成功响应已完成Agent setup并发布idle Agent、live Session及immutable header，不必等待首个prompt/`turn/start`；但`session/created`只以`void initFor(session)`启动PersistenceCoordinator，create不等待`session/flush`。`SessionPersistence.create`又明确允许直到first append才物化，所以create ok不能泛化成Session artifact/header已落介质。若payload带`workspaceId`，ApiProxy会在publication后另行`await workspace.attachSession(id)`；其domain update是durability-before-memory，但与SessionPersistence不原子，attach失败会返回`workspace-attach-failed`且已发布Session仍在。只传cwd则没有Workspace membership。
...
- 对zero-event lazy Session，一方coordinator test证明dispose可释放未物化id供后续复用；这只是该允许实现的结果，不是public abandon/delete receipt。`SessionStore` detach只移除exact live entry与publication hooks；`AgentHandle.dispose()`只保证Agent停止/idle、scope收口及Agent/Session registry detach，二者都不删除Workspace membership、archive状态或已物化Session artifact。重复create也不能改preset：同id/cwd/effective preset可adopt，不同explicit preset稳定返回`agent-preset-conflict`。
