...
## 2026-08-23 agent 复审增量（tool-bash 对 $DSH_* 的自动注入与硬承诺）

- **官方 tool-bash 会自动注入托管环境事实**：组合 `dsh-shell-env` 后，`tool-bash` 的 execute 把 `ctx.shellEnv.collect(exec)` 的结果放进 `request.dshEnv`（`packages/shell/tool-bash/src/index.ts` 约 L341-348），`dshEnv` 是 `ShellExecRequest` 的官方契约字段（`packages/shell/shell/src/types.ts` 约 L107）。
- **工具描述是硬承诺**：tool-bash 的 schema 描述原文向模型承诺 "Current harness environment facts are exposed through managed $DSH_* variables; inspect them when needed"。**自定义 ShellExecutor 宿主若在 resolve/spec 构造中丢弃 `dshEnv`**，模型仍会按官方描述去 `printenv DSH_SESSION_ID` 一类探查并得到空值，形成"给模型承诺不存在的行为"——要么把白名单后的 dshEnv 传进实际命令构造，要么明确记录与官方描述的偏差。
- 顺带：上游 tool-bash 描述与实现均写 `bash -c`/session workspace 语义；宿主若用 `bash -lc` 或按调用临时解析相对 workdir，属于与官方执行语义的未披露漂移，应在文档声明。
