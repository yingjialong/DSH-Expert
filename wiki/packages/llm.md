...
16. **exact-model metadata不承载tool catalog budget**：`LlmResolvedModelInfo`只有modalities/context/defaultMaxTokens/reasoning；没有supportsTools、tool-name grammar或max-tool-count。`session.selectModel`只resolve provider/model/reasoning，切换时不读取schemas；下一step把完整assembly交给adapter。DeepSeek与pi-ai adapters都map完整tool array，不clamp数量。
