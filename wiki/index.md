...
| [client](packages/client.md) [stale] | Web GUI 浏览器半边；alpha.2新增schedule UI package，其余结构待专项复验。 | Product | 44 | 16 |
...
| [core](packages/core.md) [stale] | 产品API主干；含Tool identity/cancel/crash、definition replacement阶段矩阵，以及static no-swap不依赖generation borrow的条件。 | Product | 8 | 23 |
...
| [mcp](packages/mcp.md) [stale] | MCP桥；含内建transport缺口、out-of-tree adapter正式形状与static no-swap边界。 | README 未列出 | 1 | 13 |
...
| [preset](packages/preset.md) [stale] | 每会话组合；含预物化content-addressed目录、standard header/cold resume与外部definition-store责任。 | Product | 2 | 17 |
...
| [session](packages/session.md) [stale] | 持久Session数据平面；alpha.2 tag下14个package manifest，固定rc.2细节回tag核验。 | Product | 14 | 32 |
...
| [skill](packages/skill.md) [stale] | Skill Registry/filesystem；含scope-local冻结provider、static无CAS条件与动态catalog缺口。 | Product | 4 | 11 |
