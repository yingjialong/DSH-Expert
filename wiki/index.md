...
> 上游基线：DSH `cd5ef814`（仓库 tag `0.1.2-alpha.1`；npm `latest` / `next` 仍为 `0.1.1-rc.2`）· Cordis `8cc9e33` · 同步于 2026-08-28
>
...
| [版本变更-0.1.1-rc.2-到-0.1.2-alpha.1.md](topics/版本变更-0.1.1-rc.2-到-0.1.2-alpha.1.md) | Host API controllers / Client Store 拓扑重组、`host-apiproxy` / `client-runtime` 删除、Remote generation stream 与 npm 仍停 rc.2 的升级边界。 | L2 | 18 |
...
| [api](packages/api.md) [stale] | Web GUI 的 Remote 栈：remotes 拿 BFF 身份策略与能力选型，gateway 实现 Host/Client 两侧共用的 Typert unary RPC 端点。 | Product | 2 | 11 |
...
| [bundle](packages/bundle.md) [stale] | dsh --profile 的可安装 patch 层：base/headless/web-app 三个在库 bundle，身份由 manifest 的 dsh.bundle.patch 定义。 | Product | 3 | 12 |
| [client](packages/client.md) [stale] | Web GUI 浏览器半边共 40 个包：两阶段 boot、React-free 对象层、slot 组合系统与 29 个 ui-* 功能插件。 | Product | 40 | 16 |
...
| [host](packages/host.md) [stale] | Web GUI 的 host 半：共享 API gateway ctx.apiProxy、裸 HTTP 承载、目录选择 seam，以及 question/approval pending registry 的 reconnect replay 语义。 | Product | 8 | 25 |
...
| [session](packages/session.md) [stale] | 持久 Session 数据平面：四条独立 seam；新增 cold list projection cache、公共 KV owner、旧日志 warm-up 与标题 UTF-8 上限的源码级边界。 | Product | 13 | 32 |
