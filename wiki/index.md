...
> 上游基线：DSH `b150a55`（0.1.1-rc.2）· Cordis `8cc9e33` · 核验于 2026-08-22
>
...
| [electron-embedding.md](integration/electron-embedding.md) | Electron/嵌入运行时集成约束：`__DSH_TRANSPORT__` carrier 正门、`/client` subpath 硬要求、`loader.internal` 缺席连锁（bareModuleBaseUrl 无效 / 官方 HMR throw）、asar 无 loader 支持与 node-pty helper 官方通道、app-boot 激活断言边界与 `--dump-config` 对照、Workspace 创建面约束、pending 不跨 host restart | L2 | 10 |
...
| [cordis-primer.md](topics/cordis-primer.md) | Cordis 内核入门：Context/Service/Plugin/Fiber/Registry/Effect 模型、ctx-key 服务解析、五种事件 dispatch mode、与 DSH tools 服务的接缝，以及 `export default` 吞 inject 等 9 条陷阱。任何 DSH 插件问题的前置知识。 | L2 | 35 |
...
| [版本变更-0.1.0-rc.8-到-0.1.1-rc.2.md](topics/版本变更-0.1.0-rc.8-到-0.1.1-rc.2.md) | 图片管线统一 + credentials 长成双 key space：5 组破坏性变更（事件拆分 / projection register 重构 / 两级限额 / 图片守卫移位 / frontend-static 404）、新增能力表、八维坐标影响与升级建议。问「升级会炸什么」的第一入口。 | L2 | 15 |
| [plugin-development.md](topics/plugin-development.md) | 插件开发全路径：从零写 dsh-plugin 的步骤路由、四种插件形态骨架、依赖纪律（Service Definition 而非 Provider）、Config/schema、事件与扩展点选择、skill provider registry、bundle/profile 发布与八条陷阱 | L2 | 35 |
| [core-chain.md](topics/core-chain.md) | agent loop 函数级调用链、agent-loop 可替换性的三道机制保证、session 三层 derive 与持久化/projection seam、会话并发隔离 12 条「条件→结论」、preset standing mount 与 scope 父链、LlmAdapter 接入点、system-prompt/context 注入时机 | L2 | 27 |
...
| [attachment](packages/attachment.md) | 图片附件 seam（ctx.attachments）：源准入（8192px/20MiB/64Mpx）→ normalized 持久化（长边 2048px）→ readImageRequest 按路由预算派生请求版本；region reads 已移除。 | Product | 2 | 9 |
...
| [credentials](packages/credentials.md) | 凭据两个 key space（CredentialRef 四方法 + CredentialKey record 五方法）+ authorization「问人要凭据」flow 包 + 本地 provider（进程环境 > .credentials.yaml > 项目 .env > 用户 .env，versioned 文档带 boot 迁移）。 | Product | 3 | 10 |
...
| [llm](packages/llm.md) | LLM seam 与两个 twin adapter（deepseek-official / pi-ai，0.1.1 起含 vision 模型与 prepareCall 同代绑定、text-only 模型图片占位投影），外加 token-meter 计量与 llm-retry 重试执行器；HarnessError 基类也住在 dsh-llm 里。 | Product | 5 | 16 |
