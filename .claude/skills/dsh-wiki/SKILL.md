...
| 检查项 | 要求 |
| --- | --- |
| frontmatter 齐全 | `status / mastery / freshness / anchors / commit / verified_at / asked_by` 七项必填 |
...
逐条复核。发现 agent 会话中**推翻过 `fact` 级条目**的（违反 `CLAUDE.md` 第 12 条），回滚该改动并记入 `errors.md`。
...
在 `wiki/coverage.md` 中查该领域当前等级，明确本次要达到的目标等级。
...
按 `CLAUDE.md` 写作规范写回，更新 `coverage.md`，然后汇报：
