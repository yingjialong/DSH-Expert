...
**消息接入与中途来信（贯穿六步）**：按 `AGENTS.md` 第 13 条先判断来源与问题归属，`steer` / follow-up 只是投递形式。明确针对已有咨询的补充、纠正或取消只更新该咨询；不同来源的新问题、同一来源的另一问题均独立登记。归属不明先确认，不合并覆盖，继续不依赖确认的工作。

在工作上下文中按“来源会话 + 咨询标识”分别保留问题、版本与完整 SHA、约束与授权、证据、进度、剩余步骤及回传目标；没有来信标识时用内部编号。无法同时处理时登记新咨询待处理，保留当前执行位置并逐项推进；受阻咨询保留状态后可切换。中途返回的工具结果仍归发起它的咨询。真实纠正及时作用于所指问题，不能把独立新咨询当作对当前咨询的取消；压缩上下文时保留未完成清单。
...
3. 涉及版本差异时，**分别说明本次目标版本与对照版本、各自固定 SHA 及行为差异**；默认基线只用于未指定版本的咨询，不得覆盖明确指定的旧版本。最新版、npm 默认版与 master 的对应关系见 `AGENTS.md` 版本基线节
...
**跨会话交付（不可省略，且是步骤 6 的硬前置）**：先核对本条咨询的来源会话、咨询标识、目标版本与 SHA，答案形成后必须立即调用宿主的跨会话回复能力，将完整、自包含的答复正文发送回其 `source_thread_id` 或等价来源会话并确认成功。仅标记本条回传状态，不将其他待处理咨询一并标为完成。确认成功前不得修改 `wiki/`、`playbooks/`、`docs/` 或其他项目文档；只在当前会话输出 final 或仅发送完成通知不算交付。调用失败时按 `CLAUDE.md` 第 13 条报告并保留答案等待重试，不得先进入沉淀。

**尾部两行（固定）**。跨会话回传发生在沉淀之前，所以第二行必须写计划状态，不得预报已经完成：

```
> 依据：本次目标版本 <版本> · 固定 <本次 SHA> · <本地关键文件绝对路径或远端 URL>
> <学习增量>
```
