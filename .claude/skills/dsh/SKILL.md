...
```bash
cd /Users/majiajun/workspace/DSH-Expert
...
3. 涉及版本差异时，**同时说明 rc.7（默认基线）与 master 的差别**
4. 禁止"可能 / 大概 / 应该是 / 通常来说"式蒙混

**尾部两行（固定）**：

```
> 依据：基线 rc.7 · 上游 <短SHA> · <关键文件路径>
> 已沉淀：<条目名> (status) · 覆盖度：<领域> Lx→Ly
```
...
- **去项目化**：剥离项目名、私有路径、业务代码、业务逻辑；无法泛化的不写
- 集成类知识写成「**条件（八维坐标）→ 结论 → 锚点 → 认知状态**」
- frontmatter 必填 `status / mastery / freshness / anchors / commit / verified_at / asked_by`
...
| `npx @deepseek-ai/dsh` 拿到的版本 | npm `latest` = rc.7，rc.8 在 `next` tag 上，需显式指定 |
