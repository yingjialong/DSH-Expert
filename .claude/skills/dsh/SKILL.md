...
**跨会话交付（不可省略）**：请求携带 `source_thread_id` 或等价来源会话标识时，答案形成后、结束当前 turn 前，必须调用宿主的跨会话回复能力，将完整、自包含的答复正文发送回该来源会话并确认成功。只在当前会话输出 final 或仅发送完成通知不算交付；调用失败时按 `CLAUDE.md` 第 13 条报告并保留答案等待重试。
